define(['exports'], (t) => {
    ...
    addCacheListener() {
      self.addEventListener('message', (t) => {
        if (t.data && 'CACHE_URLS' === t.data.type) {
          ...
          t.waitUntil(s), t.ports?.[0] && s.then(() => t.ports[0].postMessage(!0));
        }
      });
    }
  ...
  const f = (t) => [l.prefix, t, l.suffix].filter((t) => t && t.length > 0).join('-');
  const w = (t) => t || f(l.precache);
  const d = (t) => t || f(l.runtime);
  function p(t, e) {
    const s = new URL(t);
    for (const t of e) {
  ...
  class R {
    ...
    async cacheMatch(t) {
      ...
      const a = Object.assign(Object.assign({}, i), { cacheName: n });
  ...
  class v {
    ...
    handleAll(t) {
      ...
      const e = t.event;
      const s = 'string' === typeof t.request ? new Request(t.request) : t.request;
      const n = 'params' in t ? t.params : void 0;
      const i = new R(this, { event: e, request: s, params: n });
  ...
  function j(t, e) {
    ...
    const n = e !== s;
    const i = P.includes(s);
    if (!(s in (n ? IDBIndex : IDBObjectStore).prototype) || (!i && !M.includes(s))) {
      return;
    }
  ...
  class A {
    ...
    async setTimestamp(t, e) {
      ...
      const n = (await this.getDb()).transaction(S, 'readwrite', { durability: 'relaxed' });
      await n.store.put(s), await n.done;
    }
    async getTimestamp(t) {
      const e = await this.getDb();
      const s = await e.get(S, this.N(t));
    ...
    async expireEntries(t, e) {
      ...
      const a = [];
      for (const t of i) {
        await s.delete(S, t.id), a.push(t.url);
      }
      return a;
    }
    N(t) {
    ...
    async getDb() {
      return (
        this._ ||
          (this._ = await ((t, e, { blocked: s, upgrade: n, blocking: i, terminated: r } = {}) => {
  ...
  class F {
    ...
    async expireEntries() {
      if (this.O) {
        return void (this.k = !0);
      }
      this.O = !0;
      const t = this.T ? Date.now() - 1e3 * this.T : 0;
      const e = await this.P.expireEntries(t, this.B);
      const s = await self.caches.open(this.I);
      for (const t of e) {
        await s.delete(t, this.M);
      }
  ...
  async function z(t, e) {
    ...
    const i = t.clone();
    const r = { headers: new Headers(i.headers), status: i.status, statusText: i.statusText };
    const a = e ? e(r) : r;
    const o = (() => {
      if (void 0 === J) {
  ...
  class Y {
    ...
    addToCacheList(t) {
      ...
      for (const n of t) {
        ...
        const r = 'string' !== typeof n && n.revision ? 'reload' : 'default';
    ...
    install(t) {
      return H(t, async () => {
        ...
        for (const [e, s] of this.F) {
          ...
          const r = new Request(e, { integrity: n, cache: i, credentials: 'same-origin' });
          await Promise.all(
  ...
  (t.CacheFirst = class extends v {
    ...
    (t.StaleWhileRevalidate = class extends v {
      ...
      async U(t, e) {
        ...
        if (!r) {
          throw new s('no-response', { url: t.url, error: i });
        }
        return r;
      }
    }),
