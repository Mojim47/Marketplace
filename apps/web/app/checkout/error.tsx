'use client';

import { useEffect } from 'react';
import { Button } from '@/components/ui/Button';
import { useTraceId } from '@/hooks/use-trace-id';
import { emitUiEvent } from '@/lib/ui-telemetry';

export default function CheckoutError({ error, reset }: { error: Error; reset: () => void }) {
  const traceId = useTraceId();

  useEffect(() => {
    emitUiEvent('error_shown', { code: 'checkout_error' }, traceId ?? undefined);
  }, [traceId]);

  return (
    <div className="min-h-screen px-6 py-16">
      <div className="glass-card mx-auto max-w-xl rounded-3xl p-8 text-center">
        <h1 className="section-title text-2xl text-slate-900">خطا در تسویه</h1>
        <p className="mt-3 text-sm text-slate-600">
          مشکلی در اجرای فرایند پرداخت رخ داد. لطفاً دوباره تلاش کنید.
        </p>
        <p className="mt-2 text-xs text-slate-500">{error.message}</p>
        <div className="mt-6">
          <Button loading={false} onClick={() => reset()}>
            تلاش مجدد
          </Button>
        </div>
      </div>
    </div>
  );
}
