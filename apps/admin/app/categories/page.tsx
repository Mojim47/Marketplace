...
export default function AdminCategoriesPage() {
  return (
    <AdminPanelShell title="مدیریت دسته بندی" subtitle="ساختار گروه بندی محصولات برای تجربه خرید دقیق و سریع.">
        ...
        <div className="grid gap-3 md:grid-cols-2">
          {categories.map((category) => (
            <div key={category.name} className="rounded-xl bg-white/5 px-4 py-3">
              ...
              <p className="mt-1 text-xs text-slate-400">{category.count.toLocaleString('fa-IR')} کالا</p>
            </div>
          ))}
        </div>
      </div>
