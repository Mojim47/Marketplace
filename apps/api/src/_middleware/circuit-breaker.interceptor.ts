...
export class CircuitBreaker {
  ...
  async exec<T>(fn: () => Promise<T>): Promise<T> {
    ...
    if (!this.canExecute()) {
      ...
      throw new CircuitBreakerError(
        `„œ«— ${this.config.name} »«“ «” . ·ÿ›« ${retryAfter} À«‰ÌÂ œÌê—  ·«‘ ò‰Ìœ.`,
        this.config.name,
        this.state,
        retryAfter
      );
  ...
  private async withTimeout<T>(fn: () => Promise<T>, timeoutMs: number): Promise<T> {
    return Promise.race([
      ...
      new Promise<T>((_, reject) => {
        setTimeout(() => {
          reject(new Error(`œ—ŒÊ«”  Å” «“ ${timeoutMs}ms „‰ﬁ÷Ì ‘œ`));
        }, timeoutMs);
      }),
    ]);
  }
  ...
  private onSuccess(): void {
    ...
    this.logger.debug(
      `[${this.config.name}] œ—ŒÊ«”  „Ê›ﬁ - Ê÷⁄Ì : ${this.state}, „Ê›ﬁÌ ùÂ«Ì „ Ê«·Ì: ${this.consecutiveSuccesses}`
    );
  }
  ...
  private onFailure(error: Error): void {
    ...
    this.logger.warn(
      `[${this.config.name}] œ—ŒÊ«”  ‰«„Ê›ﬁ - Ê÷⁄Ì : ${this.state}, Œÿ«Â«Ì „ Ê«·Ì: ${this.consecutiveFailures}`,
      error.message
    );
  ...
  private transitionTo(newState: CircuitState): void {
    ...
    this.logger.log(
      `[${this.config.name}]  €ÌÌ— Ê÷⁄Ì  „œ«—: ${oldState} -> ${newState}`
    );
  }
  ...
  reset(): void {
    ...
    this.logger.log(`[${this.config.name}] „œ«— »Â ’Ê—  œ” Ì »«“‰‘«‰Ì ‘œ`);
  }
  ...
  trip(reason?: string): void {
    ...
    this.logger.warn(
      `[${this.config.name}] „œ«— »Â ’Ê—  œ” Ì »«“ ‘œ${reason ? `: ${reason}` : ''}`
    );
  }
}
...
export class CircuitBreakerRegistry {
  ...
  getOrCreate(config: CircuitBreakerConfig): CircuitBreaker {
    ...
    if (!circuit) {
      ...
      this.logger.log(`„œ«— ÃœÌœ «ÌÃ«œ ‘œ: ${config.name}`);
    }
        ...
        new HttpException(
          {
            ...
            message: `”—ÊÌ” ${this.circuitName} „Êﬁ « œ— œ” —” ‰Ì” . ·ÿ›« ${retryAfter} À«‰ÌÂ œÌê—  ·«‘ ò‰Ìœ.`,
