...
export class CheckoutService {
  ...
  async getSession(sessionId: string, userId: string): Promise<CheckoutSession> {
    ...
    if (!session) {
      throw new NotFoundException('Ã·”Â Å—œ«Œ  Ì«›  ‰‘œ Ì« „‰ﬁ÷Ì ‘œÂ «” ');
    }

    if (session.userId !== userId) {
      throw new BadRequestException('œ” —”Ì €Ì—„Ã«“');
    }
  ...
  async setShippingAddress(sessionId: string, userId: string, address: ShippingAddress): Promise<CheckoutSession> {
    ...
    if (!lock) {
      throw new BadRequestException('Ã·”Â Å—œ«Œ  œ— Õ«· Å—œ«“‘ «” ');
    }
  ...
  async setPaymentMethod(sessionId: string, userId: string, method: PaymentMethod): Promise<CheckoutSession> {
    ...
    try {
      ...
      if (!session.shippingAddress) {
        throw new BadRequestException('«» œ« ¬œ—” «—”«· —« Ê«—œ ò‰Ìœ');
      }
  ...
  async completeCheckout(sessionId: string, userId: string): Promise<{ orderId: string; orderNumber: string }> {
    ...
    try {
      ...
      if (!session.shippingAddress) {
        throw new BadRequestException('¬œ—” «—”«· Ê«—œ ‰‘œÂ «” ');
      }

      if (!session.paymentMethod) {
        throw new BadRequestException('—Ê‘ Å—œ«Œ  «‰ Œ«» ‰‘œÂ «” ');
      }
      ...
      const order = await this.prisma.$transaction(async (tx) => {
        ...
        for (const item of session.cartSnapshot.items) {
          ...
          if (!product || product.stock < item.quantity) {
            throw new BadRequestException(`„ÊÃÊœÌ "${item.productName}" ò«›Ì ‰Ì” `);
          }
        }
