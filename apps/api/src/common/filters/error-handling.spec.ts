...
describe('Error Handling Security', () => {
  describe('Property 11: Error Message Sanitization', () => {
    ...
    it('should sanitize database errors in all environments', () => {
      fc.assert(
        fc.property(statusCodeArb, dbErrorArb, (status, dbError) => {
    ...
    it('should never expose stack traces in error responses', () => {
      ...
      fc.assert(
        fc.property(fc.string({ minLength: 10, maxLength: 100 }), (errorMessage) => {
    ...
    it('should preserve incoming correlation ID', () => {
      fc.assert(
        fc.property(fc.uuid(), (incomingId) => {
          const mockRequest = {
  ...
  describe('Property 12: Correlation ID Propagation', () => {
    ...
    it('should isolate correlation IDs between nested contexts', () => {
      fc.assert(
        fc.property(fc.uuid(), fc.uuid(), (outerCorrelationId, innerCorrelationId) => {
          // Ensure different IDs for meaningful test
          if (outerCorrelationId === innerCorrelationId) {
            return true;
          }
    ...
    it('should support multiple correlation ID header formats', () => {
      ...
      const headerFormats = ['x-correlation-id', 'x-request-id', 'x-trace-id'];

      fc.assert(
        fc.property(fc.uuid(), fc.constantFrom(...headerFormats), (correlationId, headerName) => {
          const mockRequest = {
            url: '/api/test',
            method: 'GET',
