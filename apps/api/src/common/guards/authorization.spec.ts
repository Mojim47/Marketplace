...
function authorizeRequest(
  ...
  // No user
  if (!user) {
    return { allowed: false, error: 'ÏÓÊÑÓí ÛíÑãÌÇÒ' };
  }
...
describe('Authorization Properties', () => {
  describe('Property 1: Role Hierarchy Transitivity', () => {
    ...
    it('should deny access to higher roles', () => {
      ...
      const result2 = authorizeRequest(
        { id: 'user-1', role: 'SELLER' },
        ['ADMIN'],
      );
      ...
      fc.assert(
        fc.property(
          ...
          (requiredRole) => {
            ...
            expect(result.allowed).toBe(false);
            expect(result.error).toBe('ÏÓÊÑÓí ÛíÑãÌÇÒ');
          }
        ),
        { numRuns: 20 }
      );
  ...
  describe('Property 5: Generic Error Messages', () => {
    it('should return same error message for all denial reasons', () => {
      ...
      expect(errors[0]).toBe('ÏÓÊÑÓí ÛíÑãÌÇÒ');
    });
  });
});
