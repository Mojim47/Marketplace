...
export class CartService {
  ...
  async addToCart(userId: string, dto: AddToCartDto): Promise<Cart> {
    ...
    if (!lock) {
      throw new BadRequestException('”»œ Œ—Ìœ œ— Õ«· Å—œ«“‘ «” ° ·ÿ›« œÊ»«—Â  ·«‘ ò‰Ìœ');
    }
    ...
    try {
      ...
      if (product.stock < dto.quantity) {
        throw new BadRequestException(`„ÊÃÊœÌ ò«›Ì ‰Ì” . „ÊÃÊœÌ ›⁄·Ì: ${product.stock}`);
      }
      ...
      if (cart.items.length >= this.config.maxItems) {
        throw new BadRequestException(`Õœ«òÀ— ${this.config.maxItems} ¬Ì „ œ— ”»œ Œ—Ìœ „Ã«“ «” `);
      }
  ...
  async updateCartItem(userId: string, dto: UpdateCartItemDto): Promise<Cart> {
    ...
    try {
      ...
      if (itemIndex < 0) {
        throw new NotFoundException('¬Ì „ œ— ”»œ Œ—Ìœ Ì«›  ‰‘œ');
      }
      ...
      if (dto.quantity <= 0) {
        ...
        if (!product || product.stock < dto.quantity) {
          throw new BadRequestException(`„ÊÃÊœÌ ò«›Ì ‰Ì” . „ÊÃÊœÌ ›⁄·Ì: ${product?.stock || 0}`);
        }
  ...
  async applyDiscount(userId: string, code: string): Promise<Cart> {
    ...
    try {
      ...
      if (!discount) {
        throw new BadRequestException('òœ  Œ›Ì› ‰«„⁄ »— «” ');
      }
  ...
  async validateForCheckout(userId: string): Promise<{ valid: boolean; errors: string[] }> {
    ...
    for (const item of cart.items) {
      ...
      if (!product) {
        errors.push(`„Õ’Ê· "${item.productName}" œÌê— „ÊÃÊœ ‰Ì” `);
      } else if (!product.isActive) {
        errors.push(`„Õ’Ê· "${item.productName}" €Ì—›⁄«· ‘œÂ «” `);
      } else if (product.stock < item.quantity) {
        errors.push(`„ÊÃÊœÌ "${item.productName}" ò«›Ì ‰Ì”  („ÊÃÊœÌ: ${product.stock})`);
      }
    }
