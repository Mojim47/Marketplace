 ...
 * Lock-in #1: " ‰Â« —«Â ò‰ —· Õ«‘ÌÂ ”Êœ Ê  Ê“Ì⁄ œ— „ÕÌÿ Å—‰Ê”«‰ «Ì—«‰"
 * ???????????????????????????????????????????????????????????????????????????
 */
...
import {
  ...
  VolatilityIndexUpdateResult,
  ProductPricingUpdateResult,
  TierDiscountUpdateResult,
...
export class FactoryPricingService {
  ...
  async setTierDiscountRate(
    ...
    if (data.discountRate < 0 || data.discountRate > 0.5) {
      throw new BadRequestException(
        'Discount rate must be between 0% and 50%'
      );
    }
