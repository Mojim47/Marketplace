...
function verifyTOTP(secret: Buffer, token: string, timestamp: number, window: number = TOTP_CONFIG.WINDOW): boolean {
      ...
      fc.assert(
        fc.property(
          ...
          (timestamp) => {
            ...
            if (secret1.equals(secret2)) return; // Skip if same (extremely unlikely)
...
describe('Backup Codes Properties', () => {
  ...
  describe('Property 10: Backup Code Uniqueness', () => {
    it('should generate unique codes', () => {
      fc.assert(
        fc.property(
          fc.constant(TOTP_CONFIG.BACKUP_CODE_COUNT),
          (count) => {
            const codes = generateBackupCodes(count, TOTP_CONFIG.BACKUP_CODE_LENGTH);
