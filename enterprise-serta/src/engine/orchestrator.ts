...
export class SertaOrchestrator {
  ...
  constructor(config: SertaConfig) {
    ...
    this.vectorStore = new VectorStore(config.vectorDb.url, config.vectorDb.collection);
  ...
  private async collectAndParseCode(): Promise<CodeFile[]> {
    ...
    try {
      ...
      for (const chunk of chunks) {
        const chunkResults = await Promise.allSettled(chunk.map((file) => parser.parseFile(file)));
  ...
  private async analyzeDependencies(codeFiles: CodeFile[]) {
    ...
    try {
      ...
      spinner.succeed(
        `Built dependency graph: ${metrics.totalNodes} nodes, ${metrics.totalEdges} edges`
      );
  ...
  private async buildThreatGraph(
    codeFiles: CodeFile[],
    dependencyGraph: any
  ): Promise<ThreatGraph> {
    ...
    try {
      ...
      spinner.succeed(
        `Built threat graph: ${threatGraph.nodes.length} nodes, ${threatGraph.edges.length} edges`
      );
      ...
      if (threatGraph.vulnerabilities.length > 0) {
        const _critical = threatGraph.vulnerabilities.filter(
          (v) => v.severity === 'critical'
        ).length;
        const _high = threatGraph.vulnerabilities.filter((v) => v.severity === 'high').length;
      }
  ...
  private async runSecurityAgents(
    codeFiles: CodeFile[],
    ...
    const agentPromises = enabledAgents.map(async (agentConfig) => {
      ...
      try {
        ...
        spinner.succeed(
          `${agent.name} completed: ${agent.findings.length} findings (${executionTime}ms)`
        );
  ...
  private async performRiskAssessment(result: AnalysisResult): Promise<void> {
    ...
    try {
      ...
      spinner.succeed(
        `Risk assessment completed: Overall risk score ${result.overallRiskScore.toFixed(1)}/10`
      );
    } catch (error) {
  ...
  private calculateOverallRiskScore(result: AnalysisResult): number {
    ...
    if (allFindings.length > 0) {
      ...
      const findingsRisk =
        allFindings.reduce((sum, finding) => {
  ...
  private generateRecommendations(result: AnalysisResult): Recommendation[] {
    ...
    const _highFindings = allFindings.filter((f) => f.severity === 'high');
    ...
    const tenantIsolationFindings = allFindings.filter(
      (f) => f.title.toLowerCase().includes('tenant') || f.title.toLowerCase().includes('isolation')
    );
    ...
    const authFindings = allFindings.filter(
      (f) => f.title.toLowerCase().includes('auth') || f.title.toLowerCase().includes('guard')
    );
...
`;
  ...
  private printSummary(result: AnalysisResult): void {
    result.recommendations.slice(0, 3).forEach((rec, _i) => {
