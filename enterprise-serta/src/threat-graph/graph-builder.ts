...
import type { DependencyGraph } from '../collector/dependency-graph';
import type { VectorStore } from '../embeddings/store';
import {
  type CodeFile,
  type CriticalPath,
  EdgeType,
...
export class ThreatGraphBuilder {
  ...
  private async buildEdges(codeFiles: CodeFile[], dependencyGraph: DependencyGraph): Promise<void> {
  ...
  private createFunctionNode(file: CodeFile, func: any): ThreatNode {
    ...
    return {
      ...
      metadata: {
        ...
        businessCriticality: this.calculateBusinessCriticality(file),
        exposureLevel: func.securityRisk?.trustBoundary
  ...
  private async identifyCriticalPaths(): Promise<void> {
    ...
    const publicNodes = this.graph.nodes.filter(
      (n) => n.metadata.exposureLevel === ExposureLevel.PUBLIC
    );

    const sensitiveNodes = this.graph.nodes.filter(
      (n) => n.type === NodeType.SECRET || n.type === NodeType.DATABASE_QUERY || n.riskScore >= 8
    );
    ...
    this.graph.criticalPaths = criticalPaths.sort((a, b) => b.riskScore - a.riskScore).slice(0, 50);
  }
  ...
  private calculateOverallRiskScore(): void {
    ...
    const totalWeight = this.graph.nodes.reduce((sum, node) => sum + this.getNodeWeight(node), 0);

    this.graph.riskScore = totalWeight > 0 ? totalRisk / totalWeight : 0;

    // Apply multipliers for critical vulnerabilities and paths
    const criticalVulns = this.graph.vulnerabilities.filter(
      (v) => v.severity === SecuritySeverity.CRITICAL
    ).length;

    const highRiskPaths = this.graph.criticalPaths.filter((p) => p.riskScore >= 8).length;
  ...
  private generateVulnerabilityId(fileId: string, pattern: any): string {
    return crypto
      .createHash('md5')
      .update(`${fileId}_${pattern.type}_${pattern.line}`)
      .digest('hex');
  }
  ...
  private isSecurityRelevantFunction(func: any): boolean {
    ...
    return (
      securityKeywords.some((keyword) => func.name.toLowerCase().includes(keyword)) ||
        ...
        if (line.includes(method)) {
          ...
          endpoints.push({
            ...
            isPublic: !lines
              ...
              .some((l) => l.includes('@UseGuards') || l.includes('@Auth')),
            hasTenantIsolation: lines
              .slice(i, Math.min(lines.length, i + 10))
              .some((l) => l.includes('tenant_id') || l.includes('tenantId')),
          });
        }
      }
    }
  ...
  private identifyExternalServices(codeFiles: CodeFile[]): any[] {
    ...
    for (const file of codeFiles) {
      ...
      const externalPatterns = ['zarinpal', 'moodian', 'kavenegar', 'fetch(', 'axios.', 'http.'];
  ...
  private identifySecrets(codeFiles: CodeFile[]): any[] {
    ...
    for (const file of codeFiles) {
      const secretPatterns = file.securityPatterns.filter((p) => p.type === 'hardcoded_secret');
  ...
  private calculateEndpointRiskScore(endpoint: any): number {
    ...
    if (!endpoint.hasTenantIsolation) {
      risk += 2;
    }
  ...
  private calculateQueryRiskScore(query: any): number {
    ...
    if (!query.hasTenantFilter) {
      risk += 4; // Major risk for missing tenant isolation
    }
  ...
  private determineExposureLevel(file: CodeFile): ExposureLevel {
    ...
    if (file.path.includes('auth') || file.path.includes('guard')) {
      return ExposureLevel.AUTHENTICATED;
    }
    if (file.path.includes('admin')) {
      return ExposureLevel.PRIVILEGED;
    }
  ...
  private checkTenantIsolation(file: CodeFile): boolean {
    return (
      file.content.includes('tenant_id') ||
      file.content.includes('tenantId') ||
      file.content.includes('RLS')
    );
  }

  private checkFunctionTenantIsolation(_func: any): boolean {
  ...
  private findNodeByImport(importPath: string): ThreatNode | undefined {
    return this.graph.nodes.find(
      (node) => node.metadata.filePath?.includes(importPath) || node.name.includes(importPath)
    );
  }

  private analyzeDataFlows(_file: CodeFile): any[] {
    // Analyze data flows within the file
    return []; // Simplified for now
  }

  private identifyAuthenticationFlows(_codeFiles: CodeFile[]): ThreatEdge[] {
    return []; // Simplified for now
  }

  private identifyPrivilegeEscalationPaths(_codeFiles: CodeFile[]): ThreatEdge[] {
    return []; // Simplified for now
  }

  private identifyExposurePaths(_codeFiles: CodeFile[]): ThreatEdge[] {
    return []; // Simplified for now
  }
  ...
  private findPathsBetweenNodes(sourceId: string, targetId: string): string[][] {
    ...
    const dfs = (currentId: string, currentPath: string[]) => {
      ...
      if (visited.has(currentId) || currentPath.length > 10) {
  ...
  private calculatePathExploitability(path: string[]): number {
    ...
    for (const nodeId of path) {
      const node = this.graph.nodes.find((n) => n.id === nodeId);
      if (node) {
        if (node.metadata.exposureLevel === ExposureLevel.PRIVATE) {
          exploitability -= 3;
        }
        if (node.metadata.exposureLevel === ExposureLevel.PRIVILEGED) {
          exploitability -= 2;
        }
        if (node.metadata.tenantIsolation) {
          exploitability -= 1;
        }
      }
    }
  ...
  exportGraph(): any {
    return {
      ...
      metadata: {
        ...
        averageNodeRisk:
          this.graph.nodes.reduce((sum, n) => sum + n.riskScore, 0) / this.graph.nodes.length,
      },
    };
  }
}
