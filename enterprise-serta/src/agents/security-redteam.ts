...
export class SecurityRedTeamAgent implements Agent {
  ...
  async execute(codeFiles: CodeFile[], threatGraph: ThreatGraph): Promise<AgentFinding[]> {
  ...
  private async testAuthenticationBypass(
    codeFiles: CodeFile[],
    _threatGraph: ThreatGraph
  ): Promise<void> {
    // Find authentication-related files
    const authFiles = codeFiles.filter(
      (file) =>
  ...
  private async testPrivilegeEscalation(
    ...
    const rbacFiles = codeFiles.filter(
      (file) =>
  ...
  private async testTenantIsolationBypass(
    ...
    const queryFiles = codeFiles.filter(
      (file) =>
  ...
  private async testBusinessLogicFlaws(
    ...
    const businessFiles = codeFiles.filter(
      (file) =>
  ...
  private async testSQLInjection(codeFiles: CodeFile[], _threatGraph: ThreatGraph): Promise<void> {
    for (const file of codeFiles) {
      const sqlInjectionFlaws = file.securityPatterns.filter((p) => p.type === 'sql_injection');
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      for (const method of httpMethods) {
        if (line.includes(method)) {
          ...
          const hasGuard = lines
            .slice(Math.max(0, i - 5), i)
            .some((l) => l.includes('@UseGuards') || l.includes('@Auth'));

          // Check if it's a public endpoint
          const isPublic = lines.slice(Math.max(0, i - 5), i).some((l) => l.includes('@Public'));
      ...
      if (line.includes('role') && (line.includes('=') || line.includes('update'))) {
        if (
          !lines
            ...
            .some((l) => l.includes('authorize') || l.includes('permission') || l.includes('admin'))
        ) {
          flaws.push({
            line: i + 1,
            code: line.trim(),
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (line.includes('findUnique') || line.includes('findFirst')) {
        ...
        if (paramMatch) {
          ...
          const hasOwnershipCheck = lines
            .slice(i, Math.min(lines.length, i + 10))
            .some((l) => l.includes('user_id') || l.includes('userId') || l.includes('owner'));
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (line.includes('price') && (line.includes('req.body') || line.includes('dto.'))) {
        if (
          !lines
            ...
            .some(
              (l) => l.includes('validate') || l.includes('product.price') || l.includes('catalog')
            )
        ) {
          flaws.push({
            line: i + 1,
      ...
      if (line.includes('total') && line.includes('=')) {
        if (
          !lines
            .slice(Math.max(0, i - 5), i + 5)
            .some((l) => l.includes('calculate') || l.includes('sum') || l.includes('validate'))
        ) {
          flaws.push({
            line: i + 1,
            code: line.trim(),
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (line.includes('payment') && line.includes('status')) {
        if (line.includes('COMPLETED') || line.includes('SUCCESS')) {
          ...
          const hasGatewayVerification = lines
            .slice(Math.max(0, i - 10), i + 10)
            .some((l) => l.includes('zarinpal') || l.includes('verify') || l.includes('authority'));
      ...
      if (line.includes('payment') && line.includes('update')) {
        if (
          !lines
            .slice(Math.max(0, i - 5), i + 5)
            .some((l) => l.includes('transaction') || l.includes('lock') || l.includes('atomic'))
        ) {
          flaws.push({
            line: i + 1,
            code: line.trim(),
  ...
  private calculateMetrics(): void {
    this.metrics = {
      ...
      criticalFindings: this.findings.filter((f) => f.severity === SecuritySeverity.CRITICAL)
        .length,
  ...
  private async testDataLeakage(_codeFiles: CodeFile[], _threatGraph: ThreatGraph): Promise<void> {
    // Implementation for data leakage testing
  }

  private async testAPISecurityFlaws(
    _codeFiles: CodeFile[],
    _threatGraph: ThreatGraph
  ): Promise<void> {
  ...
  private async testCryptographicFlaws(
    _codeFiles: CodeFile[],
    _threatGraph: ThreatGraph
  ): Promise<void> {
  ...
  private async testSecretExposure(
    _codeFiles: CodeFile[],
    _threatGraph: ThreatGraph
  ): Promise<void> {
  ...
  private async testContainerEscape(
    _codeFiles: CodeFile[],
    _threatGraph: ThreatGraph
  ): Promise<void> {
  ...
  private async testNetworkSegmentation(
    _codeFiles: CodeFile[],
    _threatGraph: ThreatGraph
  ): Promise<void> {
