...
import { CodeFile, FunctionInfo, CodeEmbedding, EmbeddingType, EmbeddingMetadata, SecurityPattern } from '../types';
...
export class SemanticEmbedder {
  ...
  async embedCodeFile(codeFile: CodeFile): Promise<CodeEmbedding[]> {
    ...
    try {
      ...
      console.log(`🔢 Generated ${embeddings.length} embeddings for ${codeFile.path}`);
      return embeddings;
    } catch (error) {
  ...
  private async createSecurityPatternEmbeddings(codeFile: CodeFile): Promise<CodeEmbedding[]> {
    ...
    for (const [patternType, patterns] of patternGroups) {
      ...
      const patternText = patterns.map(p => 
        `${p.type} ${p.severity} ${p.description} ${p.recommendation}`
      ).join(' ');
  ...
  private async createFunctionEmbeddings(codeFile: CodeFile): Promise<CodeEmbedding[]> {
    ...
    const relevantFunctions = codeFile.functions.filter(func => 
      func.complexity > 5 || 
  ...
  private async createCodeEmbeddingModel(): Promise<tf.LayersModel> {
    ...
    const attention = tf.layers.dense({ units: 512, activation: 'relu' }).apply(embedding) as tf.SymbolicTensor;
    
    // Global average pooling
    const pooled = tf.layers.globalAveragePooling1d().apply(attention) as tf.SymbolicTensor;
    
    // Dense layers for final embedding
    const dense1 = tf.layers.dense({ units: 2048, activation: 'relu' }).apply(pooled) as tf.SymbolicTensor;
    const dropout = tf.layers.dropout({ rate: 0.1 }).apply(dense1) as tf.SymbolicTensor;
    const output = tf.layers.dense({ units: this.embeddingDimensions, activation: 'tanh' }).apply(dropout) as tf.SymbolicTensor;
  ...
  private initializeVocabulary(): void {
    ...
    const commonTokens = [
      ...
      'auth', 'authentication', 'authorization', 'token', 'jwt', 'password',
      'secret', 'key', 'encrypt', 'decrypt', 'hash', 'salt', 'validate',
      'sanitize', 'escape', 'csrf', 'xss', 'sql', 'injection', 'guard',
      
      // Business domain tokens (NextGen Marketplace specific)
      'order', 'payment', 'product', 'user', 'tenant', 'zarinpal', 'moodian',
      'invoice', 'executor', 'vendor', 'marketplace', 'b2b', 'b2c',
  ...
  private calculateSecurityRelevance(codeFile: CodeFile): number {
    ...
    const securityFunctions = codeFile.functions.filter(f => 
      f.name.includes('auth') || f.name.includes('validate') || f.name.includes('guard')
    );
  ...
  private calculateComplexity(codeFile: CodeFile): number {
    const avgComplexity = codeFile.functions.reduce((sum, f) => sum + f.complexity, 0) / 
  ...
  private isTrustBoundary(codeFile: CodeFile): boolean {
    const trustBoundaryPatterns = [
      'controller', 'gateway', 'resolver', 'middleware', 'guard', 'interceptor'
    ];
  ...
  private calculateDependencySecurityRelevance(codeFile: CodeFile): number {
    ...
    const externalDeps = codeFile.imports.filter(imp => 
      !imp.startsWith('.') && !imp.startsWith('@nextgen/') && !imp.startsWith('@libs/')
    );
    relevance += externalDeps.length * 0.5;
    
    // Security-related imports
    const securityImports = codeFile.imports.filter(imp => 
      imp.includes('auth') || imp.includes('security') || imp.includes('crypto')
    );
  ...
  private hasTrustBoundaryDependencies(codeFile: CodeFile): boolean {
    return codeFile.imports.some(imp => 
      imp.includes('auth') || imp.includes('security') || imp.includes('guard')
    );
  }

  private extractBusinessKeywords(codeFile: CodeFile): string[] {
    const businessKeywords = [
      'order', 'payment', 'product', 'user', 'tenant', 'vendor', 'executor',
      'invoice', 'zarinpal', 'moodian', 'marketplace', 'b2b', 'b2c',
      'pricing', 'discount', 'inventory', 'shipping', 'warranty'
    ];
