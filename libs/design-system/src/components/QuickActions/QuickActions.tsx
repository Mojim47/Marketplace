...
export const QuickActions: React.FC<QuickActionsProps> = ({
  ...
  const colorClasses = {
    ...
    secondary:
      'bg-[var(--color-brand-secondary)] hover:bg-[var(--color-brand-secondaryHover)] text-white',
  ...
  return (
    <div
      className={cn('fixed z-modal', positionClasses[position], className)}
      dir={rtl ? 'rtl' : 'ltr'}
    >
        ...
        {actions.map((action, index) => (
          ...
          >
            ...
            {hoveredAction === action.id && (
              <div
                className={cn(
                  'absolute top-1/2 -translate-y-1/2 px-3 py-1.5 rounded-lg bg-[var(--color-surface-default)] shadow-lg border border-[var(--color-border-light)] text-sm whitespace-nowrap animate-in fade-in duration-150',
