...
export const IranianPostalCodeSchema = z
  .string()
...
export const FileUploadSchema = z.object({
  ...
  size: z.number().positive().max(10 * 1024 * 1024, 'حجم فایل نباید بیش از ۱۰ مگابایت باشد'),
});
...
export const IPAddressSchema = z
  .string()
  .ip({ message: 'آدرس IP نامعتبر است' });
...
export const OrderNumberSchema = z
  .string()
  .regex(/^ORD-\d{4}-\d{6}$/, 'شماره سفارش نامعتبر است');
