...
export async function executeTransaction(prisma, operation, options = {}) {
    ...
    while (attempts < config.maxRetries) {
        ...
        try {
            ...
            logger.error(`تراکنش ناموفق - تلاش: ${attempts}, مدت: ${duration}ms`, error instanceof Error ? error.stack : String(error));
            return {
                success: false,
 ...
 * ```
 */
export async function withOptimisticLock(prisma, modelName, id, expectedVersion, operation, options = {}) {
    return executeTransaction(prisma, async (tx) => {
        ...
        if (entity.version !== expectedVersion) {
            throw new OptimisticLockError(`تغییر همزمان شناسایی شد - نسخه مورد انتظار: ${expectedVersion}, نسخه فعلی: ${entity.version}`);
        }
        // Execute operation
