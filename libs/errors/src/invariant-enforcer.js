...
exports.InvariantEnforcer = exports.ErrorInvariantEnforcer = exports.PaymentInvariantEnforcer = exports.AuthInvariantEnforcer = void 0;
...
exports.PaymentInvariantEnforcer = {
    ...
    verifyDoubleEntryBalance(entries) {
        if (invariants_1.PAYMENT_INVARIANTS.DOUBLE_ENTRY_BALANCE.signal(entries)) {
            ...
            throw errors_1.BusinessRuleError.violation('Transaction entries must balance (debits = credits)', 'مجموع بدهکاری و بستانکاری باید برابر باشد');
        }
    },
    /**
    ...
    verifyIdempotencyKeyBinding(stored, incoming) {
        if (stored && invariants_1.PAYMENT_INVARIANTS.IDEMPOTENCY_KEY_BINDING.signal(stored, incoming)) {
            logger.error(`[INV-PAY-003] Idempotency key mismatch: key=${incoming.key}`);
            throw errors_1.BusinessRuleError.violation('Idempotency key already used with different authority', 'کلید یکتاسازی قبلاً با مرجع دیگری استفاده شده است');
        }
    },
    /**
...
exports.ErrorInvariantEnforcer = {
    ...
    validateErrorCodeStatus(code, actualStatus) {
        ...
        if (expectedStatus !== actualStatus) {
            logger.warn(`[INV-ERR] Status mismatch for ${code}: expected ${expectedStatus}, got ${actualStatus}`);
            return false;
        }
        return true;
    },
