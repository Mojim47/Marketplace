...
describe('INVARIANT ENFORCEMENT', () => {
  ...
  describe('PAYMENT INVARIANTS', () => {
    ...
    it('INV-PAY-003: detects idempotency key mismatch', () => {
      ...
      expect(signal({ key: 'k1', authority: 'auth1' }, { key: 'k1', authority: 'auth2' })).toBe(
        true
      );

      // OK: same key, same authority
      expect(signal({ key: 'k1', authority: 'auth1' }, { key: 'k1', authority: 'auth1' })).toBe(
        false
      );

      // OK: different keys
      expect(signal({ key: 'k1', authority: 'auth1' }, { key: 'k2', authority: 'auth2' })).toBe(
        false
      );
    });
